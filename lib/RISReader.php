...
class RISReader {
  ...
  protected function parseArray($lines) {
    ...
    ### Modification by Brian Min - 16/03/2016 ###

    $reference_list = array();

    for($i = 0; $i < count($recordset); $i++){
      $authors = $recordset[$i]['AU'];
      $title = $recordset[$i]['T1'][0];
      if(empty($title)) $title = $recordset[$i]['TI'][0];
      $journal = $recordset[$i]['JF'][0];
      #if(strrpos($journal, ".") !== (strlen($journal) - 1)){
       # $journal .= ".";
      #}
      if(empty($journal)) $journal = $recordset[$i]['JO'][0];
      $SP = $recordset[$i]['SP'][0];
      $EP = $recordset[$i]['EP'][0];
      $VL = $recordset[$i]['VL'][0];
      $PY = $recordset[$i]['PY'][0];
      ...
      if(!empty($SP) && !empty($EP)){
        $current_reference .= $title . '. ' . $journal . ' ' . $PY . ';' . $VL . ':' . $SP . '-' . $EP;
      }else{
        $current_reference .= '(' . $PY . '). ' . $title . '. ' . $journal . ' ' . $VL . '.';
      }
